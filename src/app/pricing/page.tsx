...
export default async function PricingPage() {
    // Same admin-configured source the theme detail page and homepage use, so
    // the anchor here can't drift from what checkout actually charges.
    const { price, originalPrice, savings } = await getHeadlinePricing();
                    ...
                    <div style={{
                        ...
                        {originalPrice > 0 && (
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.75rem', marginBottom: '0.25rem' }}>
                                ...
                                <span style={{ background: '#DCFCE7', color: '#15803D', fontSize: '0.72rem', fontWeight: 700, letterSpacing: '0.06em', padding: '3px 10px', borderRadius: '100px' }}>SAVE ₹{inr(savings)}</span>
                            </div>
                        )}
                        <div style={{ marginBottom: '0.5rem' }}>
                            <span style={{ fontFamily: 'var(--font-serif)', fontSize: '4rem', fontWeight: 700, color: 'var(--secondary)', lineHeight: 1 }}>
                                <span style={{ fontSize: '2rem', verticalAlign: 'super' }}>₹</span>{inr(price)}
                            </span>
                        </div>
